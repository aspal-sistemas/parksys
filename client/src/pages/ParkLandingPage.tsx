  ...
  return (
    <PublicLayout>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {/* Banner publicitario - Ancho completo fuera del grid */}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card>
              ...
              <CardContent>
                {park.concessions && park.concessions.length > 0 ? (
                  <div className="space-y-4">
                    {park.concessions.slice(0, 3).map((concession) => (
                      <div key={concession.id} className="border rounded-lg p-4 hover:bg-gray-50">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="font-semibold">{concession.vendorName}</h4>
                          <Badge variant="outline" className="text-xs">
                            {concession.type}
                          </Badge>
                        </div>
                        <p className="text-gray-600 text-sm mb-2">{concession.description}</p>
                        <div className="flex items-center gap-4 text-sm text-gray-500">
                          <div className="flex items-center gap-1">
                            <MapPin className="h-3 w-3" />
                            {concession.location}
                          </div>
                          {concession.startDate && concession.endDate && (
                            <div className="flex items-center gap-1">
                              <Calendar className="h-3 w-3" />
                              {format(new Date(concession.startDate), 'MMM yyyy', { locale: es })} - 
                              {format(new Date(concession.endDate), 'MMM yyyy', { locale: es })}
                            </div>
                          )}
                        </div>
                        {concession.contactInfo && (
                          <div className="mt-2 text-sm">
                            ...
                            <span className="font-medium">{concession.contactInfo}</span>
                          </div>
                        )}
                      </div>
                    ))}
                    {park.concessions.length > 3 && (
                      <div className="text-center pt-4">
                        <Link href="/concessions">
                          <Button variant="outline" size="sm">
                            Ver todas las concesiones
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p>No hay concesiones activas en este momento.</p>
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                {park.volunteers && park.volunteers.length > 0 ? (
                  <div className="space-y-4">
                    {park.volunteers.slice(0, 3).map((volunteer) => (
                      <div key={volunteer.id} className="flex items-center gap-3 p-3 bg-red-50 rounded-lg">
                        <div className="w-10 h-10 bg-red-200 rounded-full flex items-center justify-center">
                          <Heart className="h-5 w-5 text-red-600" />
                        </div>
                        <div className="flex-1">
                          <h4 className="font-semibold text-red-900">{volunteer.name}</h4>
                          <p className="text-sm text-red-700">{volunteer.role}</p>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p>No hay voluntarios activos registrados.</p>
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                {park.documents && park.documents.length > 0 ? (
                  <div className="space-y-3">
                    {park.documents.map((doc) => (
                      <div key={doc.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 cursor-pointer">
                        <FileText className="h-5 w-5 text-gray-600" />
                        <div className="flex-1">
