...
// Enum para versiones del sistema
export const systemVersionEnum = pgEnum("system_version", ["municipal", "network", "pro"]);

// Enum para tipos de parque
export const parkTypeEnum = pgEnum("park_type", ["barrial", "vecinal", "urbano", "metropolitano"]);

// Enum para complejidad de plantillas
export const templateComplexityEnum = pgEnum("template_complexity", ["simple", "standard", "complex"]);
...
export const systemConfigurations = pgTable("system_configurations", {
  ...
  // Versión del sistema
  version: systemVersionEnum("version").notNull().default("municipal"),
  maxParks: integer("max_parks").notNull().default(5000),
  
  // Información municipal (solo para versión municipal)
  municipalityName: varchar("municipality_name", { length: 255 }),
  municipalityPopulation: varchar("municipality_population", { length: 100 }),
  municipalityContact: varchar("municipality_contact", { length: 255 }),
  
  // Módulos habilitados
  enabledModules: json("enabled_modules").$type<string[]>().default([]),
  
  // Configuración de tipos de parques
  parkTypeConfigurations: jsonb("park_type_configurations").$type<{
    id: string;
    name: string;
    description: string;
    maxSize?: number;
  ...
  publicPageTemplates: jsonb("public_page_templates").$type<{
    id: string;
    name: string;
    description: string;
    sections: string[];
...
// Tabla de tipos de parques (referencia para clasificación automática)
export const parkTypes = pgTable("park_types", {
  id: serial("id").primaryKey(),
  
  // Identificación
  typeKey: parkTypeEnum("type_key").notNull().unique(),
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  
  // Límites de tamaño
  minSize: decimal("min_size", { precision: 10, scale: 2 }), // en hectáreas
  maxSize: decimal("max_size", { precision: 10, scale: 2 }), // en hectáreas
  
  // Configuración de página pública
  publicPageTemplate: templateComplexityEnum("public_page_template").notNull().default("standard"),
  enabledFeatures: json("enabled_features").$type<string[]>().default([]),
  
  // Apariencia
  icon: varchar("icon", { length: 10 }).default("🏞️"),
  color: varchar("color", { length: 50 }).default("bg-green-100 text-green-800"),
...
export const systemConfigurationsRelations = relations(systemConfigurations, ({ many }) => ({
  // Puede tener múltiples configuraciones (historial)
}));

export const parkTypesRelations = relations(parkTypes, ({ many }) => ({
  parks: many(parks),
}));

// Esquemas de validación para configuración del sistema
...
export const insertParkTypeSchema = createInsertSchema(parkTypes).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type ParkType = typeof parkTypes.$inferSelect;
export type InsertParkType = z.infer<typeof insertParkTypeSchema>;
