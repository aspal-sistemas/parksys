...
import { ArrowLeftCircle, ArrowRightCircle, Plus, Calendar, ArrowLeft, Clock } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
// Datos de muestra para activos
const sampleAssets = [
  { id: 1, name: 'Banca Modelo Colonial', category: 'Mobiliario', park: 'Parque Agua Azul' },
  { id: 2, name: 'Juego Infantil Múltiple', category: 'Juegos Infantiles', park: 'Parque Agua Azul' },
  { id: 3, name: 'Fuente Central', category: 'Ornamental', park: 'Parque Agua Azul' },
  { id: 4, name: 'Estación de Ejercicio', category: 'Equipamiento Deportivo', park: 'Parque Agua Azul' },
  { id: 5, name: 'Bebedero', category: 'Servicios', park: 'Parque Agua Azul' },
  { id: 6, name: 'Luminaria Solar', category: 'Iluminación', park: 'Parque Agua Azul' },
  { id: 7, name: 'Basurero Ecológico', category: 'Mobiliario', park: 'Parque Agua Azul' }
];

// Datos de muestra para técnicos
const sampleTechnicians = [
  { id: 1, fullName: 'Carlos Jiménez', role: 'técnico' },
  { id: 2, fullName: 'Ana Gutiérrez', role: 'técnico' },
  { id: 3, fullName: 'Roberto Méndez', role: 'técnico' }
];

// Datos de muestra para mantenimientos programados
const sampleScheduledMaintenances = [
  {
    id: 1,
    assetId: 1,
    date: new Date(new Date().setDate(new Date().getDate() + 2)).toISOString(),
    maintenanceType: 'Limpieza',
    description: 'Limpieza y repintado de banca',
    estimatedCost: 500,
    priority: 'medium',
    assignedToId: 1,
    notes: 'Usar pintura especial para exteriores',
    status: 'scheduled',
    asset: { name: 'Banca Modelo Colonial' },
    assignedTo: { fullName: 'Carlos Jiménez' }
  },
  {
    id: 2,
    assetId: 2,
    ...
    description: 'Revisión de seguridad de juego infantil',
    estimatedCost: 0,
    priority: 'high',
    assignedToId: 2,
    notes: 'Revisar especialmente tornillos y soldaduras',
    status: 'scheduled',
    asset: { name: 'Juego Infantil Múltiple' },
    assignedTo: { fullName: 'Ana Gutiérrez' }
  },
  {
    id: 3,
    assetId: 3,
    date: new Date(new Date().setDate(new Date().getDate() + 5)).toISOString(),
    maintenanceType: 'Reparación',
    description: 'Reparación del sistema de bombeo de la fuente',
    estimatedCost: 3500,
    priority: 'high',
    assignedToId: 3,
    notes: 'Traer repuestos de bomba modelo H-330',
    status: 'scheduled',
    asset: { name: 'Fuente Central' },
    assignedTo: { fullName: 'Roberto Méndez' }
  },
  {
    id: 4,
    assetId: 6,
    ...
    maintenanceType: 'Inspección',
    description: 'Revisión de panel solar y batería',
    estimatedCost: 0,
    priority: 'low',
    assignedToId: 1,
    notes: 'Comprobar nivel de carga y limpieza de panel',
    status: 'scheduled',
    asset: { name: 'Luminaria Solar' },
    assignedTo: { fullName: 'Carlos Jiménez' }
  },
  {
    id: 5,
    ...
    maintenanceType: 'Limpieza',
    description: 'Limpieza y desinfección de basureros',
    estimatedCost: 300,
    priority: 'medium',
    assignedToId: 2,
    notes: 'Usar equipo de protección y desinfectante industrial',
    status: 'scheduled',
    asset: { name: 'Basurero Ecológico' },
    assignedTo: { fullName: 'Ana Gutiérrez' }
  }
];

const MaintenanceCalendarStaticPage = () => {
  const [_, setLocation] = useLocation();
  const { toast } = useToast();
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [selectedDate, setSelectedDate] = useState<Date | null>(null);
  const [maintenances, setMaintenances] = useState(sampleScheduledMaintenances);
  ...
  const onSubmit = (data: MaintenanceFormValues) => {
    const newMaintenance = {
      ...
      ...data,
      status: 'scheduled',
      asset: { name: sampleAssets.find(a => a.id === data.assetId)?.name || 'Activo desconocido' },
      assignedTo: data.assignedToId 
        ? { fullName: sampleTechnicians.find(t => t.id === data.assignedToId)?.fullName || 'Sin asignar' }
        : null
    };
  ...
  // Filtrar mantenimientos próximos (próximos 30 días)
  const upcomingMaintenances = maintenances.filter(maintenance => {
    ...
    return maintenanceDate >= now && maintenanceDate <= thirtyDaysFromNow;
  });
            ...
            <CardContent>
              <div className="space-y-4">
                {upcomingMaintenances.length > 0 ? (
                  upcomingMaintenances.slice(0, 5).map((maintenance, index) => (
                      ...
                      <div className="mt-2">
                        ...
                        {maintenance.assignedTo && (
                          <p className="text-xs text-muted-foreground mt-1">
                            Asignado a: {maintenance.assignedTo.fullName}
                          </p>
                        )}
                      </div>
                    </div>
...
export default MaintenanceCalendarStaticPage;