...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.get('/system/configuration', isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        maxParks: config.max_parks,
        enabledModules: config.enabled_modules || [],
        parkTypeConfigurations: config.park_type_configurations || [],
        publicPageTemplates: config.public_page_templates || [],
        municipalityInfo: config.version === 'municipal' ? {
          name: config.municipality_name,
          population: config.municipality_population,
          contact: config.municipality_contact
        } : undefined
      });
  ...
  apiRouter.post('/system/configuration', isAuthenticated, async (req: Request, res: Response) => {
    try {
      const {
        version,
        maxParks,
      ...
      if (!version || !maxParks) {
        return res.status(400).json({ 
          error: 'Faltan campos requeridos: version y maxParks' 
        });
      }

      // Desactivar configuración anterior
      await pool.query('UPDATE system_configurations SET is_active = false WHERE is_active = true');
      ...
      const result = await pool.query(insertQuery, [
        ...
        JSON.stringify(parkTypeConfigurations || []),
        JSON.stringify(publicPageTemplates || []),
        municipalityInfo?.name || null,
        municipalityInfo?.population || null,
        municipalityInfo?.contact || null
      ]);
      ...
      res.json({
        ...
        config: {
          ...
          maxParks: savedConfig.max_parks,
          enabledModules: savedConfig.enabled_modules || [],
          parkTypeConfigurations: savedConfig.park_type_configurations || [],
          publicPageTemplates: savedConfig.public_page_templates || [],
          municipalityInfo: savedConfig.version === 'municipal' ? {
            name: savedConfig.municipality_name,
            population: savedConfig.municipality_population,
            contact: savedConfig.municipality_contact
          } : undefined
        }
      });
      ...
      res.status(500).json({ 
        error: 'Error al guardar configuración del sistema',
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });

  // Obtener tipos de parques disponibles
  apiRouter.get('/system/park-types', isAuthenticated, async (req: Request, res: Response) => {
    try {
      const result = await pool.query(`
      ...
      res.json(result.rows.map(row => ({
        ...
        minSize: row.min_size ? parseFloat(row.min_size) : null,
        maxSize: row.max_size ? parseFloat(row.max_size) : null,
        publicPageTemplate: row.public_page_template,
      ...
      console.error('Error al obtener tipos de parques:', error);
      res.status(500).json({ 
        error: 'Error al obtener tipos de parques',
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });
