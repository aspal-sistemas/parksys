...
import { budgetProjections, BudgetMatrix, BudgetEntry, extractMonthlyValues } from '../shared/budget-planning-schema';
...
export function registerBudgetPlanningRoutes(app: any, apiRouter: Router, isAuthenticated: any) {
  // Obtener matriz presupuestaria para un año específico
  apiRouter.get('/budget-projections/:year', async (req: Request, res: Response) => {
    try {
      ...
      for (let month = 1; month <= 12; month++) {
        const monthlyIncome = budgetMatrix.incomeCategories.reduce((sum, cat) => sum + cat.months[month], 0);
        const monthlyExpense = budgetMatrix.expenseCategories.reduce((sum, cat) => sum + cat.months[month], 0);
      ...
      budgetMatrix.yearlyTotals.income = budgetMatrix.incomeCategories.reduce((sum, cat) => sum + cat.total, 0);
      budgetMatrix.yearlyTotals.expense = budgetMatrix.expenseCategories.reduce((sum, cat) => sum + cat.total, 0);
